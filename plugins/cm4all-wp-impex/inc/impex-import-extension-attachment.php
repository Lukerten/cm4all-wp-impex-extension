...
\add_action(Impex::WP_ACTION_REGISTER_PROVIDERS, __NAMESPACE__ . '\__registerAttachmentImportProvider');
...
class __AttachmentImporter
{
  ...
  function fetch_remote_file($url, $post, $slice)
  {
    ...
    copy($this->transformationContext->path . '/' . $this->slice[Impex::SLICE_DATA], $new_file);
...
\add_action('rest_api_init', function () {
  ...
  \add_filter(ImpexImportRESTController::WP_FILTER_IMPORT_REST_SLICE_UPLOAD, function (array $slice, ImpexImportTransformationContext $transformationContext, \WP_REST_Request $request) {
    if (
...
\add_action(Impex::WP_ACTION_ENQUEUE_IMPEX_PROVIDER_SCRIPT, function ($client_asset_handle, $in_footer) {
  ...
  \wp_add_inline_script($HANDLE, sprintf('wp.impex.extension.import.attachment.WP_FILTER_IMPORT_REST_SLICE_UPLOAD_FILE=%s;', \wp_json_encode(AttachmentImporter::WP_FILTER_IMPORT_REST_SLICE_UPLOAD_FILE)));
}, 10, 2);
